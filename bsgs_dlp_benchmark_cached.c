...
/* ---------------- parallel BSGS solve (giant steps) ---------------- */

typedef struct {
    const bsgs_ctx* b;                 /* shared (baby table + MG, neg_MG) */
    secp256k1_pubkey targetPm;         /* copy */
    unsigned char target33[33];        /* serialized target once */
    uint64_t j_start;                  /* inclusive */
    uint64_t j_end;                    /* exclusive */
    atomic_int* found;                 /* shared flag */
    uint64_t* found_m;                 /* shared result */
    pthread_mutex_t* found_mu;         /* guard writing found_m (optional but safe) */
} bsgs_worker_args;
...
static void* bsgs_worker_thread(void* argp) {
    ...
    /* Create a thread-local context (safe, avoids any shared-context concerns) */
    secp256k1_context* ctx = secp256k1_context_create(SECP256K1_CONTEXT_VERIFY);
    if (!ctx) return NULL;

    /* Compute j_start * MG  as (j_start*M) * G via ec_pubkey_create */
    uint64_t j = a->j_start;
    uint64_t jm = j * b->M;  /* fits since bits_total <= 63 */
    ...
    for (; j < a->j_end; j++) {
        ...
        if (pubkey_equal33(jmg33, a->target33)) {
            uint64_t m = j * b->M;
        ...
        /* lookup i>0: does Q match a baby step? */
        unsigned char q33[33];
        if (!pubkey_serialize33(ctx, &Q, q33)) break;

        uint32_t i_found;
        if (map_get(&b->baby, q33, &i_found)) {
            uint64_t m = j * b->M + (uint64_t)i_found;
            pthread_mutex_lock(a->found_mu);
            if (!atomic_load_explicit(a->found, memory_order_relaxed)) {
                *a->found_m = m;
                atomic_store_explicit(a->found, 1, memory_order_relaxed);
...
/* Parallel solve:
 * - First checks baby table for (j=0) case.
 * - Then splits j in [1, J) across threads.
 */
static int bsgs_solve_parallel(const bsgs_ctx* b,
    ...
    if (nthreads == 1) {
        return bsgs_solve(b, targetPm, out_m);
    }

    const secp256k1_context* ctx0 = b->ctx;

    /* serialize target once */
    unsigned char target33[33];
    if (!pubkey_serialize33(ctx0, targetPm, target33)) return 0;

    /* j=0 case: direct baby lookup */
    uint32_t i_found;
    if (map_get(&b->baby, target33, &i_found)) {
        *out_m = (uint64_t)i_found;
        return 1;
    }
    ...
    if ((uint64_t)nthreads > (J - 1)) nthreads = (int)(J - 1); /* cap threads */
