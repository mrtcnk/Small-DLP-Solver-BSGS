...
#include "secp256k1_gej_zaddc_var.h"
#include "secp256k1_gej_zaddU_ge_var.h"
 ...
 *
 * Build:
 *   cc -O3 -Wall -Wextra -o bsgs_zaddc bsgs_dlp_benchmark_zaddc.c \
 *       -I/usr/local/include                                   \
...
static inline void process_window_single_thread( size_t w_count,
                                   uint64_t j_start,
                                   secp256k1_gej * restrict Pm_gej, 
                                   secp256k1_gej * restrict jMG_gej,
                                   ...
                                   secp256k1_fe * restrict scratch)
{
    ...
    for (size_t w = 0; w < w_count; w++) {
        secp256k1_gej_zaddc_var(&Qj_add, &Qj_sub, Pm_gej, jMG_gej);
        ...
        secp256k1_gej_zaddU_ge_var(jMG_gej, Pm_gej, jMG_gej, &b->MG_ge, Pm_gej);
    }
    ...
    for (size_t w = 0; w < w_count; w++) {
        ...
        for (int ci = 0; ci < nc; ci++) {
            /*
             * m = ±(j*M) ± i, kept in uint64_t until one final cast to int64_t,
             * avoiding UB from early signed casts/negation. j*M <= 2^(l-1); at
             * l=64 this reaches 2^63, so subtractive forms always fit int64_t,
             * but additive forms can overflow -- only when they're not the true
             * candidate, so verify_candidate() safely rejects the aliased result.
            */
            int64_t m1 = (int64_t)(j_win[w] * b->M + (uint64_t)cands[ci]); // +j*M + i
            int64_t m2 = (int64_t)(j_win[w] * b->M - (uint64_t)cands[ci]); // +j*M - i
        ...
        for (int ci = 0; ci < nc; ci++) {
            int64_t m1 = (int64_t)((uint64_t)cands[ci] - j_win[w] * b->M); // i - j*M = -j*M + i
            int64_t m2 = (int64_t)(0ULL - (j_win[w] * b->M + (uint64_t)cands[ci])); // 0 - (j*M + i) = -j*M - i
            if (EXPECT(verify_candidate(b->ctx, m1, target33), 0)) { *out_m = m1; *found = 1; return; }
            else if (EXPECT(verify_candidate(b->ctx, m2, target33), 0)) { *out_m = m2; *found = 1; return; }
        }
    }
...
static int bsgs_solve(const bsgs_ctx* b,
    ...
    if (!secp256k1_ec_pubkey_create(b->ctx, &jMG_pub, jm_sc)) {
        free(x_add); free(x_sub); free(z_c); free(j_win); free(z_inv); free(scratch);
        return 0;
    }

    secp256k1_gej Pm_gej, jMG_gej;
    pubkey_to_gej(targetPm, &Pm_gej);
    pubkey_to_gej(&jMG_pub, &jMG_gej);
    ...
    for (uint64_t l = 0; l < full_loops; l++) {
        if (found) break;
        process_window_single_thread(W, j, &Pm_gej, &jMG_gej, b, t33, &found, out_m, x_add, x_sub, j_win, z_c, z_inv, scratch);
        j += (uint64_t)W;
    }
    /* ---- remaining steps (< W) ---- */
    if (rem > 0 && !found) {
        process_window_single_thread(rem, j, &Pm_gej, &jMG_gej, b, t33, &found, out_m, x_add, x_sub, j_win, z_c, z_inv, scratch);
        j += (uint64_t)rem;
    }
...
static void* bsgs_worker_thread(void* argp) {
    ...
    secp256k1_gej Pm_gej, jMG_gej;
    pubkey_to_gej(&a->targetPm, &Pm_gej);
    pubkey_to_gej(&jMG_pub, &jMG_gej);
        
    /* ---- windowed main loop ---- */
    for (uint64_t l = 0; l < full_loops; l++) {
        if (atomic_load_explicit(a->found, memory_order_relaxed)) break;
        process_window(ctx, W, j, &Pm_gej, &jMG_gej, b, a, x_add, x_sub, j_win, z_c, z_inv, scratch);
        j += (uint64_t)W;
    }
    /* ---- remaining steps (< W) ---- */
    if (rem > 0 && !atomic_load_explicit(a->found, memory_order_relaxed)) {
        process_window(ctx, rem, j, &Pm_gej, &jMG_gej, b, a, x_add, x_sub, j_win, z_c, z_inv, scratch);
        j += (uint64_t)rem;
    }
...
static int bsgs_solve_zaddc_parallel(const bsgs_ctx* b,
                               const secp256k1_pubkey* targetPm,
 ...
 * using `bsgs_solve_zaddc_parallel_wrapper`, which operates in [-2^(l-1), 2^(l-1)). 
 */
static int bsgs_solve_zaddc_parallel_wrapper(const bsgs_ctx* restrict b,
    ...
    found = bsgs_solve_zaddc_parallel(b,&Pm_adj_pk,threads,window,&out_m_signed);
...
static void benchmark_bsgs(int bits_total, int l1, int trials, int threads,
    ...
    for (int t = 0; t < trials; t++) {
        ...
        ts = now_seconds();
        int found = bsgs_solve_zaddc_parallel_wrapper(&solver, &Pm_pk, threads, window, &recovered);
